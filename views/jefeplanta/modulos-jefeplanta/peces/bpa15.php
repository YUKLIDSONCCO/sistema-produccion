...
<body>
  ...
  <script>
    ...
    function verListado(){
      ...
      const registro = { fecha, encargado, lote, sede, actividad, filas };

      // almacenar en localStorage para listado simulado
      const registros = JSON.parse(localStorage.getItem('bpa15_registros') || '[]');
      registros.push(registro);
      localStorage.setItem('bpa15_registros', JSON.stringify(registros));

      // redirigir al listado en PecesController
      window.location.href = 'index.php?controller=Peces&action=bpa15Listado';
    }
