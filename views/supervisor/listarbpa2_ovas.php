...
<head>
    ...
    <title>Reportes BPA-2 – OVAS</title>
    <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="bg-gray-100">

<div class="max-w-7xl mx-auto p-6">
    <h1 class="text-3xl font-bold text-gray-800 mb-6">💀 Mortalidad Diaria – Ovas</h1>

    <div class="overflow-x-auto bg-white shadow-md rounded-lg">
        <table class="min-w-full table-auto" id="tablaBpa2Ovas">
            <thead class="bg-blue-600 text-white">
                <tr>
                    <th class="px-4 py-2 text-left">ID</th>
                    <th class="px-4 py-2 text-left">Fecha Control</th>
                    <th class="px-4 py-2 text-left">Encargado</th>
                    <th class="px-4 py-2 text-left">Sede</th>
                    <th class="px-4 py-2 text-left">Lote</th>
                    <th class="px-4 py-2 text-left">Batería</th>
                    <th class="px-4 py-2 text-left">Batea</th>
                ...
                <?php if (!empty($reportes)): ?>
                    <?php foreach ($reportes as $r): ?>
                        <tr class="border-b hover:bg-gray-50">
                            ...
                            <td class="px-4 py-2"><?= $r['fecha_control'] ?></td>
                            <td class="px-4 py-2"><?= $r['encargado'] ?></td>
                            <td class="px-4 py-2"><?= $r['sede'] ?></td>
                            <td class="px-4 py-2"><?= $r['lote'] ?></td>
                            <td class="px-4 py-2"><?= $r['bateria'] ?></td>
                            <td class="px-4 py-2"><?= $r['batea'] ?></td>
                            <td class="px-4 py-2 font-bold text-red-600"><?= $r['total'] ?></td>
                            <td class="px-4 py-2"><?= $r['observacion'] ?></td>
                            <td class="px-4 py-2"><?= $r['fecha_registro'] ?></td>
                            <td class="px-4 py-2 text-center">
                                <a href="index.php?controller=Supervisor&action=bpa2Ovas&id=<?= $r['id'] ?>"
